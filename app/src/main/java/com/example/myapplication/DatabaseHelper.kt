...
class DatabaseHelper(context: Context) :
    ...
    fun addNote(note: Note): Long {
        ...
        val values = ContentValues().apply {
            // Берем данные из полей объекта Note
            put(DBContract.NoteEntry.COLUMN_NAME_TITLE, note.title)
            put(DBContract.NoteEntry.COLUMN_NAME_DESCRIPTION, note.description)
        }
        // Вставка новой строки и возврат ее ID
    ...
    fun clearAllNotes(): Int {
        val db = this.writableDatabase
        // Метод delete() возвращает количество удаленных строк
        val deletedRows = db.delete(DBContract.NoteEntry.TABLE_NAME, null, null)
        db.close()
        return deletedRows
    }
